...
///
/// Every plain field set here belongs to the type-agnostic
/// `AVHWFramesContext` `ffmpeg-sys-next` binds; FFmpeg fills in the
/// `AVVulkanFramesContext` itself during `av_hwframe_ctx_init` — images
/// that can be sampled, stored to, and copied both ways, where the format
/// allows.
///
/// # Safety
///

...
/// Errors specific to `FileDemuxSource`. Converts into the crate-wide
...
/// one of these pads.
pub struct FileDemuxSource {
    name: String,
    input: ffmpeg::format::context::Input,
    pads: Vec<SrcPad>,
}

impl FileDemuxSource {
...
impl Element for FileDemuxSource {
    fn name(&self) -> &str {
        &self.name
    }
}

impl Source for FileDemuxSource {
    fn src_pads(&mut self) -> &mut [SrcPad] {
        &mut self.pads
    }
}

impl SourceElement for FileDemuxSource {

...
| [`screen_record_software`](screen_record_software/) | Record with software conversion and encoding | Windows / Linux | DXGI / PipeWire -> system-memory BGRA | OpenH264 MP4 | Fixed duration (`Finish`) | `[output.mp4] [seconds]` plus Linux source/token |
...
| [`player`](player/) | Play a file with its sound through `Player` | Windows / Linux | `Player`: GPU decode where it can (D3D11VA / NVDEC), software where not -> `VideoWindow` + audio output | EOS, Escape or close window; Space pauses, arrows seek, `.`/`,` step a picture, F or a double click fills the screen | `<video>` |

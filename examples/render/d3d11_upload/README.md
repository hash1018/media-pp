...
frames (built via plain `windows-rs` calls + `av_buffer_create`, not FFmpeg's
own hwframe pool) are readable by `D3d11WindowRenderer`'s zero-copy path.
Compare against `d3d12_upload`, the D3D12 sibling of this same smoke test.

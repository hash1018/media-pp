...
/// `windows_example`: same source, same resampler-owns-conversion split, same
/// runtime volume changes, same playback-clock binding — only the renderer and
/// its device type differ.
#[cfg(target_os = "linux")]
mod linux_example {
    use std::{thread, time::Duration};
    ...
    use media_pp::{
        ...
        bus::BusEvent,
        elements::{
            AudioResampler, AudioVolume, PipeWireAudioRenderer, PipeWireAudioRendererOptions,
    ...
    /// TestAudioSource -> AudioResampler -> AudioVolume -> Queue ->
    /// PipeWireAudioRenderer: plays a 440Hz tone for three seconds and
    ...
    pub(super) fn run() -> Result<()> {
        ...
        let device = devices
            ...
            // A session need not designate a default; fall back to any sink

...
mod example {
    ...
    pub(super) fn run() -> media_pp::Result<()> {
        ...
        // `Mp4Muxer::add_stream`) — skip anything else (subtitles, data
        // streams, ...) rather than failing the whole remux over one stream
        // this can't carry.
        let mut muxer = Mp4Muxer::create(&output_path)?;
        ...
        // its own `Eos`, not the first one (see `Mp4Muxer::open`'s own docs on
